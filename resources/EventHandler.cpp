				  ...
				  if(current_possible[i][0]==row_no && current_possible[i][1]==col_no)
					{
							...
							//for castling to check if any rook or king moved
					if(white_castled == false || black_castled == false){
						int piece_no = current_possible[0][2];
						int coll=current_possible[0][1];
						pi1.check_if_moved(piece_no,coll,moved); 
						
					} //castling cheking end
					}
					
				}
				//for actual casteling
				if(can==1 && (col_no==2 || col_no ==6)){pi1.do_castle(row_no,col_no,game_array,current_possible[0][2]);
				if(current_possible[0][2]==5){white_castled=true;} //if castling is done checks true
				if(current_possible[0][2]==-5){black_castled=true;}}  //if castling is done checks true
			}
			//to remove all elemets of vector 
			 //	:either it finds possible moves or not:
			 current_possible.clear();
			 can=0;      //always makes castling variable 0 meaning cannot castle
				...
				else if (clicked_piece == 5 || clicked_piece == -5)
				{
					...
					//for casting, checks if catling is done already 
					//checks if any rook or king is moved
					if(white_castled==false && moved[0]==0  && clicked_piece==5){
						if(moved[4]==0){can=1;pi1.check_left(window,game_array,row_no,col_no,square,can);}
						if(moved[5]==0){can=1;pi1.check_right(window,game_array,row_no,col_no,square,can);}
					}else if( black_castled==false && moved[1]==0 && clicked_piece==-5){
						if(moved[2]==0){pi1.check_left(window,game_array,row_no,col_no,square,can);}
						if(moved[3]==0){pi1.check_right(window,game_array,row_no,col_no,square,can);}
					}
